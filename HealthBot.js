...
function getWeatherInfo(params, callback) {
  ...
  const urlString = "http://api.weather.com/v1/geocode/"+params["lati"]+"/"+params["long"]+"/observations/historical.json?apiKey=f43934a981fc48f5926e5929d3ee0760&units=e&startDate="+today;
        ...
        return new Promise((resolve, reject) => {
            ...
            getWeatherInfo(params, function(error, weather) {
                ...
                if (error) {
                    ...
                    if (string.match(/.*cloudy.*/)) {
                        query = 'sandals';
                        console.log("query sandals");
                    } else if (string.match(/.*fair.*/)) {
                        query = 'jacket';
                        console.log("query jacket");
                    } else {
                        query = 'hat';
                        console.log("query hat");
                    }
                    ...
                    getGoodzer(params, function(error, products) {
                        let prd = "<a target=\"_blank\" href="+products.stores[0].products[0].url+"><h3>"+products.stores[0].products[0].title+"</h3><img class=\"img-thumbnail\" src="+products.stores[0].products[0].image+"><br>$"+products.stores[0].products[0].price+"</a>";
                        let prd2 = "<a target=\"_blank\" href="+products.stores[0].products[1].url+"><h3>"+products.stores[0].products[1].title+"</h3><img class=\"img-thumbnail\" src="+products.stores[0].products[1].image+"><br>$"+products.stores[0].products[1].price+"</a>";
